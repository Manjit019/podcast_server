...
export const extendGraphqlSchemas = (schema: any) =>
  mergeSchemas({
    schemas: [schema],
    typeDefs: gql`
      ...
      Query: {
        getRecommendedPodcasts: async (_, { userId }, context) => {
          try {
            ...
            From the following available podcasts, suggest 3 that match their interests or if not then still any random one : 
              ${
          ...
          `;
            ...
            const aiResponse =
              resonponse.data.candidates?.[0]?.content?.parts?.[0]?.text || {};

            console.log(aiResponse);

            const jsonMatch = aiResponse.match(/```json\n([\s\S]*?)\n```/);
            if (!jsonMatch) {
              throw new Error("AI response doesn't contain JSON");
            }
